...
class authController {
    static homeInfoUpdate = async (req, res) => {
        ...
        try {
            if (email && token) {
                ...
                if (decode) {
                    const isUser = await authModel.findById(decode.userID).select("--password");
                    if (isUser && email === isUser.email){
                        return res.status(200).json({
                            message: "Updated Data Fetch Succesfully!",
